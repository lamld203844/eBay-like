...
class listing_create_form(forms.Form):
    ...
    description = forms.CharField(
        ...
        widget=forms.Textarea(
            attrs={
                "class": "form-group form-control",
                "placeholder": "Description (maximum 512 characters)",
            }
        ),
    )
    ...
    starting_bid = forms.DecimalField(
        ...
        widget=forms.NumberInput(
            attrs={
                ...
                "step": "1",
                "class": "form-group form-control",
                "placeholder": "Starting bid (in dollars round to integer)",
            }
        ),
    )
...
def create_listing(request):
    if request.method == "POST":
        ...
        form = listing_create_form(request.POST)
    ...
    return render(request, "auctions/create_listing.html", {"form": listing_create_form()})
...
def make_bid(request, auction_id):
    if request.method == "POST":
        ...
        if satisfy(bidding, auction):
            ...
            instance = bid(user=request.user, bidding=bidding, listing=auction)
            instance.save()
...
def satisfy(bidding, auction):
    if (bidding > auction.starting_bid) and (
        auction.current_bid is None or bidding > auction.current_bid
    ):
        return True
    else:
        return False
...
def category(request):
    ...
    return render(request, "auctions/category.html", {"kinds": kinds})
...
def category_detail(request, kind):
    ...
    return render(
        ...
        {"categories": categories, "kind": kind},
    )
